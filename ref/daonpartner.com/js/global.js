...
function zoomInOut(value) {
  if (
    (value == "plus" && curRate >= maxRate) ||
    (value == "minus" && curRate <= minRate)
  ) {
    return;
  }
  if (value == "plus") {
...
function openWin(url, target, width, height, scrollbars, pos, resize) {
  ...
  if (pos) {
    ...
    xy_pos +=
      ",top=" + (Math.round(screen.height / 3) - Math.round(height / 3));
  }
  ...
  for (var i = 0; i < root.childNodes.length; i++) {
    ...
    for (var j = 0; j < root.childNodes[i].childNodes.length; j++) {
      ...
      if (root.childNodes[i].childNodes[j].nodeName == "A") {
        ...
        src2menu.push(
          getSrc(
            root.childNodes[i].childNodes[j].getElementsByTagName("img")[0].src
          )
        );
        ...
        if (sub_menu) {
          ...
          for (var k in sub_menu.childNodes) {
            if (sub_menu.childNodes[k].nodeName == "LI") {
              ...
              src_item.push(
                getSrc(
                  sub_menu.childNodes[k].getElementsByTagName("img")[0].src
                )
              );
            }
          }
          ...
          for (var l in sub_menu.childNodes) {
            if (
              typeof sub_menu.childNodes[l] == "object" &&
              sub_menu.childNodes[l].nodeType == 3 &&
              sub_menu.childNodes[l].nodeName == "#text"
            ) {
          ...
          for (var k in item[j].childNodes) {
            ...
            if (item[j].childNodes[k] == this.parentNode) {
              menu[j]
                ...
                .setAttribute("src", src2menu[j].on);
              menu[j].className = "on";
              item[j].childNodes[k]
                .getElementsByTagName("img")[0]
                .setAttribute("src", src2item[j][k].on);
            } else {
              item[j].childNodes[k]
                .getElementsByTagName("img")[0]
                .setAttribute("src", src2item[j][k].off);
            }
          }
        }
      };
  ...
  var selected = function () {
    ...
    for (var i in menu) {
      if (i == s1) {
        ...
        menu[i]
          .getElementsByTagName("img")[0]
          .setAttribute("src", src2menu[i].on);
        menu[i].className = "on";
        ...
        if (item[i]) {
          ...
          for (var j in item[i].childNodes) {
            ...
            if (j == s2) {
              item[i].childNodes[j]
                .getElementsByTagName("img")[0]
                .setAttribute("src", src2item[i][j].on);
            } else {
              item[i].childNodes[j]
                .getElementsByTagName("img")[0]
                .setAttribute("src", src2item[i][j].off);
            }
          }
        }
      } else {
        menu[i]
          .getElementsByTagName("img")[0]
          .setAttribute("src", src2menu[i].off);
        menu[i].className = "";
      }
    }
  };
...
function init1DepthComboMenu2(seq) {
  ...
  for (var k = 0; k < allB.length; k++) {
    var str = allB.item(k) + "";
    if (str.substr(str.length - 10, 2) == seq) {
      allB.item(k).parentNode.className =
        allB.item(k).parentNode.className + " active";
      return false;
    }
  }
...
function initLeftMenuLI(select1, select2) {
  ...
  for (var k = 0; k < allB.length; k++) {
    ...
    if (str.substr(str.length - 10, 6) == select2) {
      allB.item(k).className += " active";
      try {
        allB.item(k).childNodes.item(0).childNodes.item(0).src = allB
          .item(k)
          .childNodes.item(0)
          .childNodes.item(0)
          .src.replace(".gif", "_on.gif");
      } catch (e) {
        console.log("");
      }
    }
...
function initBoardArticle(container_id) {
  ...
  for (var i = 0; i < h3.length; i++) {
    if (h3.item(i).id == "article_setting") {
      ...
      set.container = h3;
      set.targetElement = document.getElementById(
        h3.item(i).getElementsByTagName("a").item(0).href.split("#")[1]
      );
    ...
    article.targetElement = document.getElementById(
      h3.item(i).getElementsByTagName("a").item(0).href.split("#")[1]
    );
...
function fncScrap(arg, arg2) {
  ...
  if (
    confirm(
      "마이페이지 > 나의 스크랩에 저장되었습니다.\n지금 확인하시겠습니까?"
    )
  ) {
    returnURL = encodeURIComponent(
      "http://www.gwangju.go.kr/ease/menu.es?S=a1&M=120500000000"
    );
  }
...
function me2day(site_nm, title, url) {
  var str = site_nm + " | " + title + " " + url;
  var sitelink =
    "http://me2day.net/posts/new?new_post[body]=" + encodeURIComponent(str);
  window.open(sitelink);
}
...
function is_IE() {
  if (navigator.appName.charAt(0) == "M") return true;
  else return false;
}
...
function bmiCheck() {
  ...
  result02 =
    ...
    (((document.getElementById("bmiHeight").value / 100) *
